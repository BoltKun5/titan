...
import { AdminPageTabEnum } from "vokit_core";
...
export const AdminPage: React.FC = () => {
  ...
  return user.role !== 1 ? (
    <div className="AdminPage-401 coloredCorner">Accès refusé</div>
  ) : (
    <div className="AdminPage-container">
      <div className="AdminPage-content coloredCorner">
        <div className="AdminPage-tabSelection">
          {tabs.map((localTab) => (
            <div
              className="AdminPage-tab"
              key={localTab.id}
              onClick={() => setTab(localTab.id)}
              style={
                localTab.id === tab
                  ? { backgroundColor: "#161827", width: "calc(100% + 1px)" }
                  : {}
              }
            >
              {localTab.name}
