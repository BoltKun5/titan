...
export const CardCounterComponent: React.FC<CardCounterComponentPropsType> = ({
  ...
  return (
    <div className="Collection-Card-overlayBottom-contentElement">
      <div className="Collection-Card-overlayBottom-content-name">{label}</div>
      <div className="Collection-Card-overlayBottom-content-management">
        <button className="Collection-Card-overlayBottom-content-minus" disabled={isDisabled}
                onClick={() => modifyQuantity(card, type, 'minus')}>-
        </button>
        <input className="Collection-Card-overlayBottom-content-input" disabled={isDisabled}
               onBlur={(ev) => setQuantity(card, type, ev.currentTarget)}
               value={card?.userCardPossessions?.[0]?.[`${type}Quantity`] ?? 0} type="number"/>
        <button className="Collection-Card-overlayBottom-content-plus" disabled={isDisabled}
