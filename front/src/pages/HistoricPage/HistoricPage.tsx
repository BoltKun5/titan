...
export const HistoricPage: React.FC = () => {
  ...
  const [historicType, setHistoricType] = useState<null | 'card' | 'booster'>(null);
  ...
  const manageScroll = (event: WheelEvent) => {
    ...
    if ((event.target as Element).matches(".HistoricPage-boosterContent")) {
      event.preventDefault();
      event.stopPropagation();
      const scrollContainer = (event.target as Element).querySelector(".HistoricPage-cardList");
      (scrollContainer as HTMLElement).scrollLeft += event.deltaY;
    }
  }
  ...
  return (
    <div className="HistoricPage-container">
      <div className="HistoricPage-typeSelector coloredCorner">
        <div className="HistoricPage-type" onClick={() => setHistoricType("booster")}>
          <img src="src/assets/booster.svg" />
          <span>BOOSTERS</span>
        </div>
        <div className="HistoricPage-type" onClick={() => setHistoricType("card")}>
                  ...
                  <tbody>
                    {
                      historicList.map((historyElement, index) => {
                        const normalDiff = historyElement.newClassicQuantity - historyElement.oldClassicQuantity;
                        const reverseDiff = historyElement.newReverseQuantity - historyElement.oldReverseQuantity;
                        return (
                          <tr key={"HistoryElementKey" + index}>
                            <td style={{ width: 40 }}><img src={`src/assets/setIcons/${historyElement.cardPossession.card.cardSet.code}.png`} /></td>
                            <td>{historyElement.cardPossession.card.name}</td>
                            <td>
                              <div className={"HistoricPage-modif " + (normalDiff > 0 ? 'increase' : 'decrease')}>
                            ...
                            <td>
                              <div className={"HistoricPage-modif " + (reverseDiff > 0 ? 'increase' : 'decrease')}>
                            ...
                            <td>{dayjs(historyElement.createdAt).format('DD/MM/YYYY')}</td>
                          </tr>
                        )
                      })
                    }
        ...
        historicType === "booster" && (
          <div className="HistoricPage-boosterHistoric coloredCorner">
            <div className="HistoricPage-boosterHistoricGrid ">
              <div className="HistoricPage-cardListHover" onClick={() => { document.querySelector(".HistoricPage-boosterHistoric")?.classList.remove("filter"); document.querySelector(`.HistoricPage-boosterContainer.opened`)?.classList.remove("opened"); }} />
              {
                boosterHistoric.map((booster: any, index: number) => {
                  return (
                    <div className="HistoricPage-boosterContainer" key={"boosterContainer" + index} id={"booster-" + booster.id} onClick={(ev) => { ev.currentTarget.classList.add("opened"); document.querySelector(".HistoricPage-boosterHistoric")?.classList.add("filter") }}>
                      <div className="HistoricPage-booster">
                        <div className="HistoricPage-boosterContent">
                          <div className="HistoricPage-cardList">
                            {
                              booster.cards.map((card: { name: string, localId: string, type: "reverse" | "classic" }, index: number) =>
                                <div className={"HistoricPage-boosterCardContainer"} key={booster.id + " " + card.localId + " " + index}>
                                  {card.type === 'reverse' && <div className="HistoricPage-boosterCardReverseOverlay reverseShining" />}
                                  <img src={getImageFromSeparatedInfos(card, booster.cardSet)} />
                                </div>
                              )
                            }
                          </div>
