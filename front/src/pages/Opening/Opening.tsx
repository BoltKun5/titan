...
export const Opening: React.FC = () => {
  ...
  let setList: LocalCardSet[] = [];
  series.map((serie) => {
    setList = setList.concat(serie.cardSets.map((_serie) => {
      return {
        name: _serie.name,
        id: _serie.id,
        code: _serie.code,
  ...
  useEffect(() => {
    ...
    fetch('cardlist/cards', { setFilter: cardSet?.code ?? '', order: 'default', page: -1 }).then((response) => {
  ...
  return (
    <div className="OpeningPage">
      <div className="OpeningPage-description coloredCorner">
        ...
        <span>Paragraphe de description de l'outil. Lorem ipsum, dolor sit amet consectetur adipisicing elit. Provident mollitia incidunt asperiores omnis quos quod,
          et odio obcaecati. Eos veniam molestiae nam neque quis repellendus provident voluptas temporibus possimus officiis.</span>
              ...
              <div className="OpeningPage-setChoice">
                ...
                <div className="OpeningPage-flex">
                  <div className="OpeningPage-setChoiceInputContainer">
                    <input className="OpeningPage-setChoiceInput" type={"text"} value={cardSetSearch} onChange={(ev) => handleChange(ev)} />
                  </div>
                ...
                <div className={"OpeningPage-setList"}>
                  {
                    setList.filter(getFilteredSets).map((set) => <div key={"openingModuleSetList" + set.code}
                      className={"OpeningPage-set" + ((set.id === cardSet?.id) ? ' selected' : '')}
                      onClick={() => {
                        setCardSet(set);
                      }}>
                ...
                <div className="OpeningPage-nextButton" style={!cardSet ? { pointerEvents: 'none' } : {}} onClick={() => setStep(step + 1)}>
            ...
            <div className="OpeningPage-swippingContent swipping2">
              <div className="OpeningPage-boosterContentInput">
                <div className="OpeningPage-boosterContent-features coloredCorner">
                  <div style={{ margin: "25px 10px 10px 10px", flex: "0 0 auto" }}>
                  ...
                  <div className="OpeningPage-boosterContent-textInput">
                    <input type={"text"} value={cardLocalId}
                      onChange={(ev) => {
                  ...
                  <div className={"OpeningPage-typeSelect"}>
                    <SwipeCheckboxComponent callback={setCardType} elements={swipeCheckboxTypeElements} value={cardType} width={100} />
                    <div>
                      <div className="OpeningPage-boosterContent-validate" onClick={() => { processNewLocalId() }}>
                        <Done />
                      </div>
                    </div>
                  </div>
              ...
              <div className="OpeningPage-boosterContentOthers">
                ...
                <div className="OpeningPage-boosterContent coloredCorner">
                  ...
                  <div className="OpeningPage-nextButton2" style={cardList.filter((el) => el.card === null).length !== 0 ? { pointerEvents: 'none' } : {}} onClick={() => submit()}>
                    <ButtonComponent label="Continuer" disabled={cardList.filter((el) => el.card === null).length !== 0} />
                  </div>
                </div>
                  ...
                  <div className="OpeningPage-possibleCards coloredCorner">
                    {
                      possibleCards.map((card, index) =>
                        <div className="OpeningPage-possibleCard" key={"OpeningPage-setCard" + index} onClick={() => {
                          setCardLocalId("")
                          setIsWrongId(false)
                          setIsPrevalidated(false)
                          setPreselectedCard(null)
                          ...
                          setCurrentIndex((currentIndex + 1 < cardList.length) ? (currentIndex + 1) : currentIndex)
                        }}
              ...
              <div className="OpeningPage-end coloredCorner">
                ...
                <div className="OpeningPage-addedCards">
                  {
                    cardList.map((value, index) =>
                      <div
                        className={"OpeningPage-addedCard " + (value.type === "reverse" ? "reverse" : "")}>
                ...
                <div className="OpeningPage-end-buttonContainer">
                  ...
                  <div onClick={() => {
                    ...
                    setCardList(initialCardList)
                    setCurrentIndex(0)
                  }}><ButtonComponent label="Ouvrir un booster du même set" size={350} /></div>
                </div>
              </div>
            </div>
          )
